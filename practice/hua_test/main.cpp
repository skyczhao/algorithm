...
int subfunc(int num, int level)
{
    ...
    if(matrix[t_level][num >> 1] != 0)
        return matrix[t_level][num >> 1];

    int t_sum = 0;
    int t_size = num / level;
    for(int t_i = 0; t_i <= t_size; t_i++){
        int t_num = num - t_i * level;
        if(matrix[t_level - 1][t_num >> 1] == 0){
            matrix[t_level - 1][t_num >> 1] = subfunc(t_num, level / 2);
        }
        t_sum += matrix[t_level - 1][t_num >> 1];
        t_sum %= DIGITS;
    }
