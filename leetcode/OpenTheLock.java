 ...
 * 752. 打开转盘锁
 * https://leetcode-cn.com/problems/open-the-lock/solution/bfsjie-ti-by-oshdyr-5w06/
 *
 * @author tobin
 * @since 2021-06-25
 */
    ...
    public int openLock(String[] deadends, String target) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int alter : alters) {
                int next = toNext(curr, alter);
                if (deads.contains(next) || counts.containsKey(next)) {
//                    continue;
                    // pass
                } else {
                    queue.add(next);
                    counts.put(next, nextCount);
                }

                int back = backNext(curr, alter);
                if (deads.contains(back) || counts.containsKey(back)) {
//                    continue;
                    // pass
                } else {
                    queue.add(back);
                    counts.put(back, nextCount);
                }
            }
        }
    ...
    private int toNext(int value, int pos) {
        int digit = (value % (pos * 10)) / pos;
        if (digit == 9) {
            return value - 9 * pos;
        } else {
            return value + pos;
    ...
    private int backNext(int value, int pos) {
        int digit = (value % (pos * 10)) / pos;
        if (digit == 0) {
            return value + 9 * pos;
        } else {
